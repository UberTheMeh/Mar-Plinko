...
public class TwitchIRC : MonoBehaviour
{
    ...
    public GameObject plinkoManager = null;
    ...
    private IEnumerator PrepareConnection()
    {
        ...
        {
            CustomEvent.Trigger(plinkoManager, "Error", "Missing required details! Check your Twitch details.");
    ...
    private void ConnectIRC()
    {
        ...
        if (!client.Connected)
        {
            CustomEvent.Trigger(plinkoManager, "Error", "Failed connecting to Twitch IRC.");
    ...
    private void Disconnect(bool reconnect = false)
    {
        ...
        CustomEvent.Trigger(plinkoManager, "Normal", "Disconnected from Twitch IRC");
        ...
        using (StreamReader reader = new StreamReader(stream)) 
        {
            ...
            while (connected)
            {
                ...
                if (settings.debugIRC)
                    CustomEvent.Trigger(plinkoManager, "Normal", "<color=#005ae0><b>[IRC INPUT]</b></color> " + raw);
    ...
    private void HandleRPL(string type)
    {
        switch (type)
        {
            case "001":
                ...
                CustomEvent.Trigger(plinkoManager, "Success", "Connected to Twitch IRC.Now joining channel: " + twitchDetails.channel + "...");
                ConnectionStateAlert(StatusType.Success, "Connected to Twitch IRC. Now joining channel: " + twitchDetails.channel + "...", 100);
                break;
            case "353":
                CustomEvent.Trigger(plinkoManager, "Success", "<color=#bd2881><b>[JOIN]</b></color> Joined channel: " + twitchDetails.channel + " successfully");
    ...
    private void HandlePRIVMSG(string ircString, string tagString)
    {
        ...
        CustomEvent.Trigger(plinkoManager, "Chatter",new Chatter(privmsg, tags));
    }
    ...
    public void ConnectionStateAlert(StatusType state, string message, int percentage = 0)
    {
        switch (state) 
        {
            case StatusType.Error:
                CustomEvent.Trigger(plinkoManager, "Error", "<color=red><b>[ERROR]</b></color>: " + message);
                Debug.LogError("<color=red><b>[ERROR]</b></color>: " + message);
                break;

            case StatusType.Normal:
                CustomEvent.Trigger(plinkoManager, "Normal", "<color=#0018a1><b>[STATUS]</b></color>: <b>" + percentage + "%</b> " + message);
                Debug.Log("<color=#0018a1><b>[STATUS]</b></color>: <b>" + percentage + "%</b> " + message);
                break;

            case StatusType.Success:
                CustomEvent.Trigger(plinkoManager, "Success", "<color=#0ea300><b>[SUCCESS]</b></color>: <b>" + percentage + "%</b> " + message);
